...
0. **默认 ponytail 模式（最高优先级）** — 用户成本敏感，token = 钱。每次写代码前先问"最少能写多少行"，禁止抽象、禁止预想未来需求、禁止写 fallback 合成逻辑、禁止防御性代码。除非用户明确说"做兼容"或"为以后准备"，否则只解决当前任务
...
Agent 收到修改指令后：
1. 只改了代码库中**一部分**位置就说"完成了"
2. 用户审查发现大量遗漏 → 再改
3. 再说"完成了" → 还是遗漏
4. 审查本身也不到位，漏看问题

**根本原因：** Agent 没有在修改前穷举所有需要改的位置，也没有在交付前做逐文件验证。

---

## Skill 使用透明度协议（强制！）

**用户无法看到你是否调用了 skill。因此你必须主动声明。**

### 规则 1：任务开始时声明

收到任何非 trivial 任务后，在回复的**第一行**必须输出：
...
例如：
```
🎯 Skills: code-search, engineering-discipline, checklist, self-verify
...
- **禁止声明用了但实际没用** — 如果说用了 code-search，就必须真的执行了搜索命令
- **禁止用了不声明** — 用了什么 skill 就写什么，不能漏

### 规则 3：用户可以随时追问

如果用户问"你用了哪些 skill""你真的搜了吗"，必须：
1. 列出使用的 skill 名称
2. 提供证据（执行的命令、搜索结果、读过的文件）

### 规则 4：如果没有使用任何 skill

```
🎯 Skills: 无（此任务为简单问答，不需要 skill）
```

**禁止什么都不写。** 必须显式声明"无"或列出具体 skill。
...
用 `code-search` skill 或 grep 工具搜索整个代码库，找出**所有**需要修改的位置：

```
🔍 修改前扫描
========================
指令：把所有 API 调用从 HTTP 改成 HTTPS

全局搜索 "http://" 在代码库中的出现位置：
1. src/api/client.ts:12 — API base URL
2. src/api/client.ts:45 — websocket URL
3. src/config/index.ts:8 — 环境配置
4. src/utils/upload.ts:23 — 文件上传地址
5. src/components/VideoPlayer.tsx:56 — 视频流地址
6. .env.example:3 — 环境变量示例
7. next.config.js:15 — 重定向配置
...
```
⚠️ 已找到 7 处，但我可能遗漏了以下情况：
- 动态拼接的 URL（如 `http://${host}/api`）可能搜不到
- 配置文件中可能有环境变量间接引用
建议：我先修改这 7 处，然后做一轮全局验证扫描确认。
```
...
📋 修改清单（共 7 处）
========================
✅ 1. src/api/client.ts:12 — API base URL → 改为 https://
✅ 2. src/api/client.ts:45 — websocket URL → 改为 wss://
✅ 3. src/config/index.ts:8 — 环境配置 → 改为 https://
⬜ 4. src/utils/upload.ts:23 — 文件上传地址 ← 正在改
⬜ 5. src/components/VideoPlayer.tsx:56 — 视频流地址
⬜ 6. .env.example:3 — 环境变量示例
⬜ 7. next.config.js:15 — 重定向配置
```
...
**规则：**
- 每改完一处 → 立即打勾
- 如果某处改不了 → 标记 ❌ 并说明原因，不要跳过不提
- 如果修改过程中发现了新的需要改的位置 → **加入清单**，不要默默改了不算
...
✅ 1. src/api/client.ts:12 — 已修改，证据: git diff 显示 http→https
✅ 2. src/api/client.ts:45 — 已修改，证据: git diff 显示 ws→wss
✅ 3. src/config/index.ts:8 — 已修改，证据: git diff 显示 http→https
✅ 4. src/utils/upload.ts:23 — 已修改，证据: git diff 显示 http→https
✅ 5. src/components/VideoPlayer.tsx:56 — 已修改，证据: git diff 显示 http→https
✅ 6. .env.example:3 — 已修改，证据: git diff 显示 http→https
❌ 7. next.config.js:15 — 未修改，原因: 该行已被删除，无需改

清单完成: 6/7 已修改，1 处无需修改
```

#### 验证 2：反向搜索确认

**用 grep/搜索工具反向检查：还有没有遗漏？**
...
结果：
- src/api/client.ts:89 — 发现残留！但这是外部 CDN 地址，不需要改 ✅
- node_modules/ — 忽略 ✅
- public/ — 静态资源，不需要改 ✅
...
修改清单: 7 处发现，6 处已修改，1 处无需修改
反向验证: 无遗漏
构建验证: 通过

修改的文件:
1. src/api/client.ts (+2 -2)
2. src/config/index.ts (+1 -1)
3. src/utils/upload.ts (+1 -1)
4. src/components/VideoPlayer.tsx (+1 -1)
5. .env.example (+1 -1)
```
...
文件 1/5: src/api/client.ts
  - diff 行数: +2 -2
  - 审查: [具体看了什么]
  - 问题: 无 / [具体问题]
  - 状态: ✅ 通过

文件 2/5: src/config/index.ts
  - diff 行数: +1 -1
  - 审查: [具体看了什么]
  - 问题: 无 / [具体问题]
  - 状态: ✅ 通过
...
| 禁止行为 | 正确做法 |
|---------|---------|
| 只改了第一个看到的文件就说完成 | 全局搜索所有位置，逐个改完再报告 |
| 说"应该都改了"但没有证据 | 列出每个文件的 git diff 证据 |
| 说"我检查了一下没问题" | 逐文件审查并列出具体看了什么 |
| 发现新位置默默改了不加入清单 | 加入清单并打勾 |
| 某处改不了就直接跳过不提 | 标记 ❌ 并说明原因 |
| 说"基本完成""差不多好了" | 报告精确数字: X/Y 已完成 |
| 反向搜索发现遗漏但不说 | 立即补改，不隐瞒 |
| 审查只看"主要文件" | 审查所有被修改的文件，一个不漏 |
...
1. 用户说"把所有 XX 改成 YY" → 必须全局搜索
2. 用户说"修改 XX 功能" → 必须搜索所有相关文件
3. 用户说"审查一下" → 必须逐文件审查
4. 修改涉及多个文件 → 必须用清单追踪
5. 用户说"你漏了" → 立即重新全局搜索，不要只补用户提到的那一处

**特别注意第 5 条：** 用户指出遗漏后，不要只补用户提到的那一处。**必须重新做全局搜索**，因为如果漏了一处，很可能还漏了更多。
...
| Skill | 用途 |
|-------|------|
| **engineering-discipline** | 核心：手术刀式修改、三层一致性检查、反合理化 |
| **code-search** | 修改前全局搜索所有受影响位置 |
| **code-review-checklist** | 审查时逐项检查，不遗漏 |
| **audit-code** | 安全审查：硬编码密钥、危险调用 |
| **refactor-safely** | 重构时小步提交、先测后动 |
| **quality-gates** | 质量门禁：pre-commit / pre-merge 检查 |
| **self-verify** | 交付前自检：来源、矛盾、不确定性 |
| **checklist** | 多步任务追踪：串行/并行执行 |
| **instant-execution-discipline** | 防止假完成：kickoff ≠ completion |
| **task-protection** | 任务全生命周期追踪 |
