...
<div class="reminder-box">이다（是）→ 이었어요/였어요（过去"是"）。학생이었어요（曾经是学生）/ 가수였어요（曾经是歌手）。이다 过去时容易忘，要单独记。</div>`,
    ...
    specialQuiz: {
      ...
      questions: [
        { prompt: '现在时：가요', options: ['가었어요', '갔어요', '갔아요'], answer: 1, explanation: '가+았어요→갔어요（缩约）' },
        { prompt: '现在时：먹어요', options: ['먹어었어요', '먹었어요', '먹았어요'], answer: 1, explanation: '먹+었어요' },
        { prompt: '现在时：해요', options: ['했어요', '해었어요', '하었어요'], answer: 0, explanation: '하+였어요→했어요（缩约）' },
        { prompt: '现在时：와요', options: ['오었어요', '와었어요', '왔어요'], answer: 2, explanation: '오+았어요→왔어요（缩约）' },
      ],
    },
...
<div class="reminder-box">거예요 前面有空格：먹을 거예요（✓），먹을거예요（✗）。거 和 예요 之间也有空格，是三个词：먹을 / 거 / 예요。</div>`,
    ...
    specialQuiz: {
      ...
      questions: [
        ...
        { prompt: '词典形：먹다', options: ['먹ㄹ 거예요', '먹을 거예요', '먹거예요'], answer: 1, explanation: '词干 먹 有收音 → 을 거예요' },
        { prompt: '词典形：하다', options: ['하을 거예요', '하ㄹ거예요', '할 거예요'], answer: 2, explanation: '词干 하 无收音 → ㄹ 거예요' },
        { prompt: '词典形：읽다', options: ['읽거예요', '읽을 거예요', '읽ㄹ 거예요'], answer: 1, explanation: '词干 읽 有收音 → 을 거예요' },
      ],
    },
...
<div class="reminder-box">두 원 ✗ → 이 원 ✓（韩元用汉字）　이 명 ✗ → 두 명 ✓（人数用固有）</div>`,
  ...
  // ── 第二章 第5课：의, 도, 만 ──────────────────────────────
  {
    ...
    mistakes: [
      { wrong: '저의의 이름은 김민수예요.', correct: '제 이름은 김민수예요.', note: '의 不能叠用（저의의 ✗）。저의 本身已表示"我的"，再加 의 是重复。口语中 제 比 저의 更自然。' },
...
<div class="reminder-box">이 사람이 에서：前面 이 是指示词（这），后面 이 是主格助词——两个 이 功能完全不同。</div>`,
  ...
  // ── 第二章 第8课：부터…까지, 时间表示法 ────────────────────
  {
    ...
    mistakes: [
      ...
      { wrong: '삼 시', correct: '세 시', note: '几点用固有数词：하나→한, 둘→두, 셋→세。삼 是汉字数词，说时刻不能用汉字数词，所以 三点 = 세 시。' },
...
<div class="reminder-box">昨天的普通事件用 -었어요 就够了，-았었어요 用于有时间距离的回忆或强调"现在已不同"。</div>`,
  ...
  },
  {
    ...
    mistakes: [
      ...
      { wrong: '배고파요. 그러면 밥을 안 먹었어요.', correct: '배고파요. 그런데 밥을 안 먹었어요.', note: '그러면 是顺接"那么/那样的话"，转折"但是"应该用 그런데 或 하지만。두 词意思完全不同，不能混用。' },
...
<div class="reminder-box">口诀：遇 ㄴ/ㅂ/ㅅ 开头语尾就脱落，遇元音开头语尾就保留。살다 → 살아요（保）/ 사세요（落）。</div>`,
  ...
  },
  {
    ...
    mistakes: [
      ...
      { wrong: '알았겠어요', correct: '알겠어요', note: '알겠어요 是 알다 + 겠 + 어요 的正确形式。알았겠어요 是把 알았다 和 겠 错误合并的结果，正确说法是 알겠어요。' },
    ],
...
<div class="reminder-box">도움 주세요 ✗ → 도와주세요 ✓ — 帮忙是固定合写形。기다리어 주세요 ✗ → 기다려 주세요 ✓ — ㅣ+어→여 缩合。-지요? 口语缩为 -죠?，意思完全相同。</div>`,
    ...
    mistakes: [
      ...
      { wrong: '어렵지 않아요?', correct: '어렵지요?', note: '-지요? 是确认疑问"很难吧？"；-지 않아요? 是否定疑问"不难吗？"。当你想确认"很难对吧"时，用 어렵지요? 而非 어렵지 않아요?。' },
      { wrong: '도움 주세요.', correct: '도와주세요.', note: '도움 是名词"帮助"，不能直接加 주세요。请求帮忙要说 도와주세요，是 돕다 + 아 주다 的固定合写形。' },
...
<div class="reminder-box">세 시에부터 ✗ → 세 시부터 ✓ — 부터 直接加在时间词后，不加 에。세 시에 만나요（在三点见）vs 세 시부터 공부해요（从三点开始学）— 一个是时刻，一个是起点。</div>`,
    ...
    mistakes: [
      ...
      { wrong: '세 시 만나요.', correct: '세 시에 만나요.', note: '时间点后必须加 에。세 시 만나요 缺少 에，是中文式直译。正确说法是 세 시에 만나요。' },
    ],
    quickTable: {
      title: '시간 표현표',
      headers: ['表达', '助词', '例子'],
...
<div class="reminder-box">지하철을 갈아타요（换乘地铁，强调对象）= 지하철로 갈아타요（换乘地铁，强调方向）— 两种说法都正确，口语中 (으)로 갈아타다 更常用。어디서 갈아타요?（在哪换乘？）是问路必备句型。</div>`,
    ...
    mistakes: [
      ...
      { wrong: '처음으로 버스를 갈아타요.', correct: '처음으로 버스를 타요.', note: '타다 是"乘坐"（首次坐），갈아타다 是"换乘"（中途换车）。第一次坐公交用 타요，갈아타요 仅用于换乘场景。' },
