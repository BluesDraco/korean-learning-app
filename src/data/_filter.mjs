...
const dictStems = new Set(dictArr.stems);
...
console.log('Ending count:', ALL.length);
...
function baseOk(p){
  ...
  if (dictSet.has(p) || dictSet.has(p+'다') || dictSet.has(p+'하다') || dictSet.has(p+'이다') || dictSet.has(p+'되다') || dictStems.has(p)) return true;
...
fs.writeFileSync('_suspicious3.txt', susp.join('\n'), 'utf8');

const delMap = new Map();
for (const w of [...dictSet].filter(w=>w.length>=2&&w.length<=6)){
  for (let i=0;i<w.length;i++){
    const k = w.slice(0,i)+w.slice(i+1);
    let a = delMap.get(k); if (!a){a=[]; delMap.set(k,a);} a.push(w);
  }
}
console.log('delMap keys:', delMap.size);
const candidates = [];
for (const t of susp){
  ...
  for (let i=0;i<t.length;i++){
    ...
    if (a) for (const w of a) if (w.length===t.length && w!==t) close.add(w);
  }
  if (close.size) candidates.push({t, close:[...close]});
}
console.log('Edit-distance-1 candidates:', candidates.length);
fs.writeFileSync('_candidates.json', JSON.stringify(candidates));
for (const c of candidates.slice(0,50)) console.log(c.t, '->', c.close.join('/'));
