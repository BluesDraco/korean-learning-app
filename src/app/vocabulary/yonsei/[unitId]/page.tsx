...
export default function YonseiUnitPage() {
  ...
  const saveSentence = async (korean: string, chinese: string, sourceTitle: string) => {
    ...
    if (!existing) {
      await db.sentences.add({ id: crypto.randomUUID(), korean, chinese, source_type: 'vocabulary', source_id: 'yonsei-' + unitId, source_title: sourceTitle, created_at: Date.now() }).catch(() => {});
    }
