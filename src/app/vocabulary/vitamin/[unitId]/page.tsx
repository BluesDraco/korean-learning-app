...
import { loadVitaminUnit, loadVitaminIndex, type UnitMeta } from '@/lib/dataLoader';
...
export default function VitaminUnitPage() {
  const { lang } = useLang();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        recordVocabVisit({ source: 'vitamin', unitId: found.id, unitTitle: `${found.bookTitle} · ${found.title}` });
  ...
  const saveSentence = async (korean: string, chinese: string, sourceTitle: string) => {
    ...
    if (!existing) {
      let addFailed = false;
      await db.sentences.add({ id: crypto.randomUUID(), korean, chinese, source_type: 'vocabulary', source_id: 'vitamin-' + unitId, source_title: sourceTitle, created_at: Date.now() }).catch((e) => { console.error('saveSentence: failed to add sentence', korean, e); addFailed = true; });
      if (addFailed) return;
    }
      ...
      for (const w of filteredWords) {
        ...
        if (!existing) {
          ...
          toInsert.push({
            ...
            nextReview: now, createdAt: now, lastReviewed: null, source: 'vitamin',
          });
      ...
      try {
        ...
        if (existing) {
          ...
          await db.words.put({
            ...
            nextReview: now + 21 * 86400000, createdAt: now, lastReviewed: now, source: 'vitamin',
          });
        }
  ...
  if (!unit) {
    return (
      <div className="py-4 space-y-4">
        <Link href="/vocabulary/library?tab=vitamin" className="inline-flex items-center gap-1.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
  ...
  return (
    <div className={isWideViewport ? 'py-6 w-full px-8 space-y-5 pb-10' : 'py-4 max-w-2xl mx-auto px-4 space-y-4 pb-[calc(56px+env(safe-area-inset-bottom,0px))]'}>
      ...
      <div style={{ marginBottom: 20 }}>
        <Link
          href="/vocabulary/library?tab=vitamin"
          style={{
        ...
        >
          <ArrowLeft size={14} />
          {t('vocab.unit_back_vitamin', lang)}
        </Link>
      ...
      {/* Flashcard entry */}
      <Link
        href={`/vocabulary/vitamin/${unitId}/flashcards`}
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-3 gap-3">
            ...
            <Link
              href={mastered > 0 ? `/vocabulary/vitamin/${unitId}/mastered` : '#'}
            ...
            <Link
              href={untouched > 0 ? `/vocabulary/vitamin/${unitId}/flashcards?filter=new` : '#'}
              className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-3 text-center block"
              ...
              <>
                ...
                >
                  {(close) => (
                    <>
                      <Link
                        href={`/vocabulary/vitamin/${unitId}/mastered`}
                        onClick={close}
                  ...
                  <div className="px-4 pb-4 border-t border-[var(--border-color)] pt-3 space-y-2 animate-slide-up">
                    ...
                    {w.examples && w.examples.length > 0 && w.examples.map((ex, i) => (
                      <div key={i} className="bg-[var(--bg-input)] rounded-lg p-3 flex items-start gap-2">
                        <div className="flex-1 min-w-0">
                          <TappableText text={ex.text} className="text-[15px] leading-relaxed text-[var(--text-primary)]" source="维他命词库" highlightWord={w.word} />
      ...
      {!loading && (mastered + learning > 0) && (() => {
        ...
        const nextUnitId = `vitamin-${bookNum}-${unitNum + 1}`;
        const nextUnitEntry = allUnits.find(u => u.id === nextUnitId);
        const nextBookId = `vitamin-${bookNum + 1}-1`;
            ...
            {isComplete ? (
              <>
                ...
                <div className="flex gap-2 justify-center flex-wrap pt-1">
                  {nextUnitEntry && (
                    <Link
                      href={`/vocabulary/vitamin/${nextUnitId}`}
                  ...
                  {!nextUnitEntry && nextBookEntry && (
                    <Link
                      href={`/vocabulary/vitamin/${nextBookId}`}
                      className="px-4 py-2 rounded-xl text-sm font-semibold text-white"
