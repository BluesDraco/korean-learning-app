...
export default function VitaminMasteredPage() {
  const { lang } = useLang();
  ...
  const [masteredWords, setMasteredWords] = useState<VitaminWord[]>([]);
  ...
  const unmaster = async (w: VitaminWord) => {
    const now = Date.now();
    ...
    <div className="min-h-screen pb-[calc(40px+env(safe-area-inset-bottom,0px))]" style={{ background: 'var(--color-surface-1)' }}>
      <div className="sticky top-0 z-10" style={{ background: 'var(--color-surface-1)', borderBottom: '1px solid var(--color-border-1)' }}>
        <div className={isDesktop ? 'max-w-5xl mx-auto' : ''} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px' }}>
          <button onClick={() => router.push(`/vocabulary/vitamin/${unitId}`)} style={{ padding: 6, background: 'transparent', border: 'none', color: 'var(--color-ink-3)', cursor: 'pointer' }}>
            <ArrowLeft size={20} />
          </button>
