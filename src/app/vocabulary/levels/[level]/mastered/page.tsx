  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#fffbf7' }}>
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 border-b border-[var(--border-color)]" style={{ background: '#fffbf7' }}>
        ...
        <h1 className="text-base font-bold text-[var(--text-primary)] flex-1">已掌握单词 · {level}级</h1>
        {masteredWords.length > 0 && (
          <button onClick={() => { setManaging(m => !m); setSelected(new Set()); setDeletePending(false); setUnmasterPending(false); }} className="text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)]">
            {managing ? '取消' : '批量管理'}
          </button>
        )}
      </div>
      ...
      {managing && masteredWords.length > 0 && (
        <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border-color)]" style={{ background: '#fffbf7' }}>
          <button onClick={() => setSelected(allSelected ? new Set() : new Set(masteredWords.map(e => e.korean)))} className="text-sm text-[var(--pink-primary)] font-medium">
            {allSelected ? '取消全选' : '全选'}
          </button>
