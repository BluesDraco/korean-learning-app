...
import { loadSeoulUnit } from '@/lib/dataLoader';
...
function SeoulFlashcardsContent() {
  ...
  useEffect(() => {
    let cancelled = false;
    loadSeoulUnit(unitId).then((found) => {
      if (cancelled) return;
      setUnit(found ?? null);
  ...
  useEffect(() => {
    ...
    if (unit === null) { router.replace('/vocabulary/library?tab=seoul'); return; }
  ...
  if (!unit || words.length === 0) {
    return (
      <div className="py-4 space-y-4 px-4">
        <Link href={`/vocabulary/seoul/${unitId}`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-muted)' }}>
  ...
  return (
    ...
    >
      ...
      <div className="no-touch-zone flex items-center gap-2 mb-4 shrink-0">
        <Link href={`/vocabulary/seoul/${unitId}`} style={{ color: 'var(--text-muted)' }}>
          <ArrowLeft size={20} />
        </Link>
                ...
                {word.examples && word.examples.slice(0, 2).map((ex, i) => (
                  <div key={i} className="rounded-[14px] px-3.5 py-2" style={{ background: 'var(--fc-example-bg)' }}>
                    <div className="flex items-start gap-2">
                      ...
                      <SentenceBookmarkButton korean={ex.text} chinese={ex.translation} sourceId={`seoul-${unitId}`} sourceTitle={unit.title} />
                      <button
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(0,0,0,.4)' }}>
          <div style={{ background: 'var(--color-surface-2)', borderRadius: 24, padding: '32px 28px 24px', maxWidth: 340, width: '100%', textAlign: 'center', boxShadow: '0 8px 40px rgba(0,0,0,.15)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => router.push(`/vocabulary/seoul/${unitId}`)} style={{ flex: 1, padding: '12px 0', borderRadius: 999, border: 'none', background: 'var(--color-pink-base)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
