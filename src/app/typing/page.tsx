...
function TargetChars({ target, input, theme }: { target: string; input: string; theme: string }) {
  const chars = Array.from(target);
  const typed = Array.from(input);
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, alignItems: 'flex-end', flex: 1 }}>
      {chars.map((ch, i) => {
        let charColor = theme === 'dark' ? '#4a4060' : '#d4c5be';
        let underColor = theme === 'dark' ? '#3A3060' : '#eee0d8';
        let isCurrent = false;
        ...
        if (i < typed.length) {
          ...
          charColor = theme === 'dark' ? '#F0E8FF' : '#241917';
          underColor = '#ff7fa8';
          isCurrent = true;
        }
...
export default function TypingPage() {
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '8px 0 100px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          {[{ label: '单词', value: previewWords.length }, { label: '句子', value: previewSents.length }, { label: '合计', value: items.length }].map(s => (
            <div key={s.label} style={{ flex: 1, background: C.card, borderRadius: 14, border: `1px solid ${C.line}`, padding: '12px 8px', textAlign: 'center' }}>
              <p style={{ fontSize: 20, fontWeight: 900, color: C.ink, margin: 0 }}>{s.value}</p>
              <p style={{ fontSize: 11, color: C.muted, margin: '2px 0 0' }}>{s.label}</p>
            </div>
          ))}
        </div>
        {previewWords.length > 0 && (
          <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.line}`, padding: '16px', marginBottom: 14 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.muted, margin: '0 0 12px', letterSpacing: '0.05em' }}>将要练习的单词</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {previewWords.map((item, i) => (
                <div key={i} style={{ background: C.pinkSoft, borderRadius: 10, padding: '6px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: C.ink, fontFamily: "'Malgun Gothic', sans-serif" }}>{item.korean}</span>
                  <span style={{ fontSize: 10, color: C.muted }}>{item.chinese}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {previewSents.length > 0 && (
          <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.line}`, padding: '16px', marginBottom: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.muted, margin: '0 0 12px', letterSpacing: '0.05em' }}>将要练习的句子</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {previewSents.map((item, i) => (
                <div key={i} style={{ borderLeft: `3px solid ${C.mint}`, paddingLeft: 10 }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: C.ink, margin: '0 0 2px', fontFamily: "'Malgun Gothic', sans-serif" }}>{item.korean}</p>
                  <p style={{ fontSize: 12, color: C.muted, margin: 0 }}>{item.chinese}</p>
                </div>
              ))}
            </div>
          </div>
        )}
        <div style={{ background: C.line, borderRadius: 12, padding: '10px 14px', marginBottom: 20, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button onClick={startPack} style={{ padding: '14px 0', borderRadius: 16, background: C.black, color: '#fff', fontSize: 15, fontWeight: 800, border: 'none', cursor: 'pointer' }}>
            开始打字 →
          </button>
          <button onClick={() => setPageState('home')} style={{ padding: '12px 0', borderRadius: 16, background: C.line, color: C.muted, fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
            返回
          </button>
        </div>
      </div>
  ...
  if (pageState === 'result' && resultData) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 20, padding: '20px' }}>
        <div style={{ background: C.pinkSoft, borderRadius: '50%', width: 80, height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Trophy size={36} style={{ color: C.pink }} />
        </div>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 13, color: C.muted, margin: '0 0 4px' }}>完成主题包</p>
          <p style={{ fontSize: 22, fontWeight: 900, color: C.ink, margin: 0 }}>{resEmoji} {resName}</p>
        </div>
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[{ label: '正确率', value: `${resultData.accuracy}%` }, { label: 'WPM', value: String(resultData.wpm) }, { label: '用时', value: resDisplay }].map(stat => (
            <div key={stat.label} style={{ background: C.card, borderRadius: 14, border: `1px solid ${C.line}`, padding: '12px 20px', textAlign: 'center', minWidth: 80 }}>
              <p style={{ fontSize: 22, fontWeight: 900, color: C.ink, margin: 0 }}>{stat.value}</p>
              <p style={{ fontSize: 11, color: C.muted, margin: '2px 0 0' }}>{stat.label}</p>
            </div>
          ))}
        </div>
        <div style={{ background: C.mintBg, borderRadius: 14, padding: '10px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 340, marginTop: 8 }}>
          <button onClick={startPack} style={{ padding: '13px 0', borderRadius: 14, background: C.black, color: '#fff', fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
            再练一次
          </button>
          <button onClick={() => setPageState('home')} style={{ padding: '13px 0', borderRadius: 14, background: C.line, color: C.muted, fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
            返回主题
          </button>
        </div>
      </div>
  ...
  if (pageState === 'session' && current) {
    const cardBorderColor = submitted ? (lastCorrect ? '#aee3d8' : '#ff7fa8') : C.line;
    const cardBg = submitted ? (lastCorrect ? '#f0faf8' : '#fff5f7') : C.card;
          ...
          <div style={{ display: 'flex', gap: 3, flex: 1 }}>
            {items.map((_, i) => (
              <div key={i} style={{
                ...
                background: i < index ? C.pink : i === index ? '#ffb8cf' : C.line,
                boxShadow: i < index ? '0 0 5px #ff7fa840' : 'none',
                animation: i === index ? 'pulse-block 1s infinite' : 'none',
              }} />
            ))}
          </div>
          <span style={{ fontSize: 12, color: C.muted, whiteSpace: 'nowrap' }}>{index + 1} / {items.length}</span>
          <span style={{ fontSize: 12, color: C.muted, fontVariantNumeric: 'tabular-nums' }}>{elapsedDisplay}</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{
              ...
              color: showTranslation ? C.muted : 'transparent',
              background: showTranslation ? 'transparent' : C.line,
              borderRadius: 6, transition: 'all 0.15s', padding: showTranslation ? 0 : '2px 8px',
            }}>
              {current.chinese}
            </span>
            <button onClick={toggleTranslation} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: C.muted, background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 8px', borderRadius: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <TargetChars target={current.korean} input={input} theme={theme} />
            <button
              onClick={() => speakWord(current.korean, 0.85)}
              style={{ flexShrink: 0, width: 42, height: 42, borderRadius: '50%', background: C.pinkSoft, border: `1.5px solid #ffd6e5`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}
            >
          ...
          <span style={{
            ...
            color: current.type === 'word' ? C.pink : '#3aafa9',
            background: current.type === 'word' ? C.pinkSoft : C.mintBg,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{
                  ...
                  border: `2px solid ${C.line}`, fontSize: 20, color: C.ink,
                  outline: 'none', fontFamily: "'Malgun Gothic', 'Apple SD Gothic Neo', sans-serif",
                  boxSizing: 'border-box', background: C.card, caretColor: C.pink,
                  transition: 'border-color 0.2s, box-shadow 0.2s',
                }}
                onFocus={e => { e.target.style.borderColor = C.pink; e.target.style.boxShadow = '0 0 0 4px #ff7fa815'; }}
                onBlur={e => { e.target.style.borderColor = C.line; e.target.style.boxShadow = 'none'; }}
              />
              <span style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: C.line, borderRadius: 6, padding: '3px 7px', fontSize: 10, color: C.muted }}>↵</span>
            </div>
            <p style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>请切换系统键盘为韩语后输入</p>
            ...
            <button
              ...
              style={{
                ...
                background: input.trim() ? C.black : C.line,
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      <div style={{ marginBottom: 16 }}>
        <Link href="/tools" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: C.muted, textDecoration: 'none', marginBottom: 10 }}>
          <ArrowLeft size={15} /> 返回
        </Link>
        <h1 style={{ fontSize: 20, fontWeight: 900, color: C.ink, margin: 0 }}>韩文打字</h1>
        <p style={{ fontSize: 13, color: C.muted, marginTop: 4, marginBottom: 0 }}>选择主题包，边打字边学词汇</p>
      </div>

      {streak > 0 && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: C.pinkSoft, borderRadius: 20, padding: '6px 14px', marginBottom: 20 }}>
          <Flame size={14} style={{ color: C.pink }} />
          <span style={{ fontSize: 13, color: C.pink, fontWeight: 700 }}>连续练习 {streak} 天</span>
        </div>
      )}
      ...
      {categories.map(cat => {
        ...
        return (
          <div key={cat} style={{ marginBottom: 28 }}>
            <h2 style={{ fontSize: 13, fontWeight: 700, color: C.muted, margin: '0 0 10px', letterSpacing: '0.05em' }}>{cat}</h2>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${progress ? C.mint : C.line}`,
                      background: progress ? C.mintBg : unlocked ? C.card : C.bg,
                  ...
                  >
                    ...
                    <p style={{ fontSize: 13, fontWeight: 700, color: C.ink, margin: '0 0 4px', lineHeight: 1.3 }}>{theme.name}</p>
                    <p style={{ fontSize: 11, color: C.muted, margin: 0 }}>{wordCount}词 · {sentCount}句</p>
                    {progress && (
      ...
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, color: C.muted, margin: '0 0 10px', letterSpacing: '0.05em' }}>我的单词本</h2>
        {wordBooks.length === 0 ? (
          <p style={{ fontSize: 13, color: C.muted }}>暂无单词本，请先在词汇页添加单词。</p>
        ) : (
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 6 }}>
            {wordBooks.map(book => (
              <button
                ...
                style={{
                  flexShrink: 0, width: 140, padding: '14px 12px', borderRadius: 16,
                  border: `1.5px solid ${C.line}`, background: C.card, cursor: 'pointer',
                  textAlign: 'left', boxShadow: '0 2px 12px rgba(78,52,46,.06)',
                }}
              >
                <div style={{ fontSize: 26, marginBottom: 6 }}>📖</div>
                <p style={{ fontSize: 13, fontWeight: 700, color: C.ink, margin: '0 0 4px', lineHeight: 1.3 }}>{book.name}</p>
                <p style={{ fontSize: 11, color: C.muted, margin: 0 }}>{book.wordIds.length} 个词</p>
              </button>
            ))}
          </div>
        )}
      ...
      {user && mySentenceCount !== null && mySentenceCount > 0 && (
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: C.muted, margin: '0 0 10px', letterSpacing: '0.05em' }}>我的句子</h2>
          <div style={{ display: 'flex', gap: 10, paddingBottom: 6 }}>
            <button
              onClick={openMySentences}
              style={{
                flexShrink: 0, width: 140, padding: '14px 12px', borderRadius: 16,
                border: `1.5px solid ${C.mint}`, background: C.mintBg, cursor: 'pointer',
                textAlign: 'left', boxShadow: '0 2px 12px rgba(78,52,46,.06)',
              }}
            >
              <div style={{ fontSize: 26, marginBottom: 6 }}>🔖</div>
              <p style={{ fontSize: 13, fontWeight: 700, color: C.ink, margin: '0 0 4px', lineHeight: 1.3 }}>我收藏的句子</p>
