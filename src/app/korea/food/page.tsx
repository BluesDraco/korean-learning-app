...
import { foodItems, type FoodItem } from '@/data/korea';
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((food) => (
            <div
              key={food.id}
              className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 space-y-3"
            >
              {/* Food header */}
              ...
              {food.ingredients.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {food.ingredients.map((ingredient) => (
                    <span
                      key={ingredient}
                      className="text-[14px] px-2 py-1 rounded-lg bg-[var(--bg-card-hover)] text-[var(--text-secondary)] border border-[var(--border-color)]"
                    >
                      {ingredient}
                    </span>
                  ))}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {food.phrases.map((phrase, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-3 bg-[var(--bg-input)] rounded-xl px-3 py-2.5"
                      >
