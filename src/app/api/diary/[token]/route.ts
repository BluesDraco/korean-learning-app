...
// 6-26 事故兜底：所有暴露用户私人数据的 API 必须 force-dynamic + private cache
...
export async function GET(
  ...
  // token 长度 + 字符集白名单（base64url / hex），防 SQL 注入残留 + 暴力扫描噪声
  if (!token || token.length < 8 || token.length > 64 || !/^[A-Za-z0-9_-]+$/.test(token)) {
    return NextResponse.json({ error: 'Invalid token' }, { status: 404, headers: { 'Cache-Control': 'private, no-store' } });
  }
  ...
  try {
    ...
    if (!linkRow || !linkRow[2]) {
      return NextResponse.json({ error: 'Link not found or inactive' }, { status: 404, headers: { 'Cache-Control': 'private, no-store' } });
    }

    const userId = linkRow[0] as string;
    const expiresAt = linkRow[1] as number | null;
    if (expiresAt && Date.now() > expiresAt) {
      return NextResponse.json({ error: 'Link expired' }, { status: 410, headers: { 'Cache-Control': 'private, no-store' } });
    }
    ...
    if (!profileRow || !profileRow[8]) {
      return NextResponse.json({ error: 'Profile not found or not public' }, { status: 404, headers: { 'Cache-Control': 'private, no-store' } });
    }
    ...
    return NextResponse.json({
      ...
      totalWords: (wordStats[0] as number) ?? 0,
      masteredWords: (wordStats[1] as number) ?? 0,
      reviewCount,
      studyDays,
      chatCount,
      booksCount,
    ...
    console.error('[api/diary/token] failed', err);
    return NextResponse.json({ error: 'Internal error' }, { status: 500, headers: { 'Cache-Control': 'private, no-store' } });
  }
}
