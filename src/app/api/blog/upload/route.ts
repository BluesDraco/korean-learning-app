...
const MAX_BYTES = 500 * 1024; // 500KB；客户端 compressImage 已缩放降质到 480KB 内，此为服务端硬保险

// POST — 发帖配图上传。返回可读 URL。
export async function POST(req: Request) {
  ...
  // 存到 data/uploads/blog/（部署受保护目录）。文件名 {userId}-{timestamp}.jpg，防目录穿越 + 一人可多图。
  const dir = path.join(process.cwd(), 'data', 'uploads', 'blog');
  await fs.mkdir(dir, { recursive: true });
  // userId 现为 randomUUID（hex+连字符），已匹配读取路由白名单；净化是纵深防御，
  // 防未来 userId 生成方式变更混入非法字符导致写成功但读 404（裂图）。
  const safeUid = auth.userId.replace(/[^a-zA-Z0-9-]/g, '');
  const fileName = `${safeUid}-${Date.now()}.jpg`;
  await fs.writeFile(path.join(dir, fileName), buf);

  const url = `/api/blog/upload/${fileName}`;
