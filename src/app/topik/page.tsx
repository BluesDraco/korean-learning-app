...
export default function TopikPage() {
  ...
  const LIGHT_C = { ink: '#241917', muted: '#89756e', line: '#eee0d8', pink: '#ff7fa8', pinkSoft: '#fff0f5', bg: '#fffbf7', mint: '#aee3d8', mintBg: '#eaf8f5', card: '#fff' };
  const DARK_C  = { ink: '#F0E8FF', muted: '#B8A8C8', line: '#3A3060', pink: '#ff7fa8', pinkSoft: '#2D2848', bg: '#1E1B2E', mint: '#4A6058', mintBg: '#1E3530', card: '#282440' };
      ...
      <div style={{ padding: '16px 16px 0' }}>
                  ...
                  {topikExamSets.filter(s => s.year === year).map(set => (
                    <div key={set.id} style={{ background: C.card, borderRadius: 14, border: `1px solid ${C.line}`, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, opacity: set.available ? 1 : 0.55 }}>
                      ...
                      {set.available ? (
                        ...
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: '#f0ece8', color: C.muted }}>即将上线</span>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {tab === 'practice' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {([['all', '全部'], ['beginner', 'TOPIK I'], ['intermediate', '中级'], ['advanced', '高级']] as const).map(([key, label]) => (
                  <button key={key} onClick={() => setPracticeLevel(key)} style={{ padding: '6px 14px', borderRadius: 20, border: `1px solid ${practiceLevel === key ? C.ink : C.line}`, background: practiceLevel === key ? C.ink : '#fff', color: practiceLevel === key ? '#fff' : C.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {([['all', '全部'], ['listening', '🎧 听力'], ['reading', '📖 阅读']] as const).map(([key, label]) => (
                  <button key={key} onClick={() => setPracticeSection(key)} style={{ padding: '6px 14px', borderRadius: 20, border: `1px solid ${practiceSection === key ? C.ink : C.line}`, background: practiceSection === key ? C.ink : '#fff', color: practiceSection === key ? '#fff' : C.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={startPractice} disabled={practiceQuestions.length === 0} style={{ width: '100%', padding: '14px 0', borderRadius: 14, background: practiceQuestions.length === 0 ? '#c4a89e' : C.ink, color: '#fff', fontSize: 14, fontWeight: 700, border: 'none', cursor: practiceQuestions.length === 0 ? 'not-allowed' : 'pointer' }}>
