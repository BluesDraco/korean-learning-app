...
export async function getDb(): Promise<Db> {
  ...
  await c.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_phone_unique ON users(phone) WHERE phone != ''`);
      ...
      `);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_clips_source_type ON shadowing_clips(source_type)`);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_clips_difficulty ON shadowing_clips(difficulty)`);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_clips_published ON shadowing_clips(is_published, sort_order)`);
      ...
      await c.execute(`
        CREATE TABLE IF NOT EXISTS shadowing_segments (
          ...
          chinese       TEXT DEFAULT '',
          tokens        TEXT DEFAULT '[]',
          shadowing_tip TEXT DEFAULT '',
          vocab_pills   TEXT DEFAULT '[]',
          created_at    INTEGER NOT NULL,
          FOREIGN KEY (clip_id) REFERENCES shadowing_clips(id) ON DELETE CASCADE
        )
      `);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_segments_clip ON shadowing_segments(clip_id, seg_index)`);
      ...
      await c.execute(`
        CREATE TABLE IF NOT EXISTS shadowing_progress (
          ...
          user_id          TEXT NOT NULL,
          clip_id          TEXT NOT NULL,
          completed_segs   TEXT NOT NULL DEFAULT '[]',
          last_seg_index   INTEGER NOT NULL DEFAULT 0,
          status           TEXT NOT NULL DEFAULT 'not_started',
          recorded_count   INTEGER NOT NULL DEFAULT 0,
          saved_word_count INTEGER NOT NULL DEFAULT 0,
          completed_at     INTEGER,
          last_studied_at  INTEGER NOT NULL,
          created_at       INTEGER NOT NULL,
          updated_at       INTEGER NOT NULL,
      ...
      `);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_shadowing_progress_user ON shadowing_progress(user_id)`);

  // ── KPOP songs cache (P1 local karaoke, global shared, permanent) ──
      await c.execute(`
        CREATE TABLE IF NOT EXISTS kpop_songs_cache (
          id TEXT PRIMARY KEY,
      ...
      `);
      await c.execute(`CREATE INDEX IF NOT EXISTS idx_kpop_songs_cache_key ON kpop_songs_cache(song_key)`);

  // ── Spelling / sentence mistakes (错题本) ──
      await c.execute(`
        CREATE TABLE IF NOT EXISTS spelling_mistakes (
          id TEXT PRIMARY KEY,
          user_id TEXT NOT NULL,
