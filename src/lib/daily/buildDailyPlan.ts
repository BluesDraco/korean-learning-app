...
export async function getUserCourseDay(): Promise<number> {
  const [events, words] = await Promise.all([
    ...
    db.words.where('source').equals('course').toArray().catch(() => []),
  ]);
  return deriveCourseDay(events, words);
}

export async function buildDailyPlanFromApi(): Promise<DailyPlan> {
  const res = await fetch('/api/daily-plan', { credentials: 'include' });
  if (!res.ok) return buildDailyPlan();
  ...
  const data = await res.json() as {
    ...
    readingCount: number;
    courseWords: { sourceDetail: string | null }[];
    completeEvents: { dayNum: number | null; timestamp: number }[];
    outputEvents: { timestamp: number }[];
    todayStart: number;
  };

  const { wordsReviewed, dueCount, pronunciationCount, readingCount,
    courseWords, completeEvents, outputEvents, todayStart } = data;

  const courseDay = deriveCourseDay(
    completeEvents.map(e => ({ dayNum: e.dayNum ?? undefined })),
    courseWords.map(w => ({ source: 'course', sourceDetail: w.sourceDetail ?? undefined }))
  );
  const course = thirtyDayCourse[courseDay - 1] || null;
  const srsDone = wordsReviewed > 0 || dueCount === 0;
  const pronunciationDone = pronunciationCount > 0;
  const readingDone = readingCount >= 3;
...
export async function buildDailyPlan(): Promise<DailyPlan> {
  ...
  const [
    ...
    db.words.where('source').equals('course').toArray().catch(() => []),
    db.words.where('nextReview').belowOrEqual(now).count().catch(() => 0),
    db.learningEvents.where('action').equals('complete').toArray().catch(() => []),
  ...
  const courseDay = deriveCourseDay(completeEvents, courseWords);
  const course = thirtyDayCourse[courseDay - 1] || null;
