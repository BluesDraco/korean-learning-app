...
export function saveLineRecording(songId: string, lineIndex: number, blob: Blob, durationMs: number): KpopLineRecording {
  ...
  try {
    localStorage.setItem(recordingKey(songId, lineIndex), JSON.stringify({
      ...
      // blobUrl is transient — regenerated from server on next load
    }));
  } catch { /* quota exceeded */ }

  // Primary persistence: upload to server
  uploadRecordingToServer(songId, lineIndex, blob, durationMs);
